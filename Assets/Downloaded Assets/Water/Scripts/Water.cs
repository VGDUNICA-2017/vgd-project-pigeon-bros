...
public class Water : MonoBehaviour {
	...
	public float WidthDef = 20; 
	public float TopDef = 0; 
	public float BottomDef = -3;

	void Start()
	{
		//Spawning our water
		SpawnWater(LeftDef,WidthDef,TopDef,BottomDef);
	}
	...
	public void Splash(float xpos, float velocity)
	{
		...
		if (xpos >= xpositions[0] && xpos <= xpositions[xpositions.Length-1])
		{
			...
			var psMain = splash.GetComponent<ParticleSystem>().main;
			psMain.startSpeed = 8+2*Mathf.Pow(Mathf.Abs(velocity),0.5f);
			psMain.startSpeed = 9 + 2 * Mathf.Pow(Mathf.Abs(velocity), 0.5f);

...
public class VibratingGround : MonoBehaviour {
	...
	bool toVibrate, toFallDown, toRaiseUp, startCount, canVibrate;
	float timer;
	...
	void OnCollisionEnter(Collision other) {
		if (!canVibrate) {
			if (other.transform.root.CompareTag ("Player")) {
				if (other.transform.root.GetComponent<Animator> ().GetFloat ("Speed") != 0)
					canVibrate = true;
			}
		}

		if (canVibrate && other.transform.root.CompareTag ("Player") && !toVibrate && !toFallDown && !toRaiseUp) {
			toVibrate = true;
		}
	}
}
